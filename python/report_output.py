...
def failure_report(out, provenance, index, meta, sequences):
    ...
    payload = dict(schema_version="1.0", provenance=provenance, metadata=meta,
        ...
        containment=read_tsv(out/"containment_candidates.tsv") if (out/"containment_candidates.tsv").is_file() else [],
        typing_crossreference=read_tsv(out/"typing_crossreference.tsv") if (out/"typing_crossreference.tsv").is_file() else [],
        multilayer_edges=read_tsv(out/"network.multilayer_edges.tsv") if (out/"network.multilayer_edges.tsv").is_file() else [],
